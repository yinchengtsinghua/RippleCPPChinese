...
//如果未指定任何内容，则默认为几个著名的大容量
//要用作引导程序的服务器：
    if (bootstrapIps.empty ())
    {
//Ripple Labs Inc.运营的服务器池-https://ripple.com
        bootstrapIps.push_back("r.ripple.com 51235");

//合金网络运营的服务器池-https://www.alloy.ee
...
OverlayImpl::stop()
{
//调用list_[].second->stop（）可能导致修改list_u
//（可以在此同一线程上调用overlayImpl:：remove（））。所以
//直接遍历list_uuo call child->stop（）可能导致
//未定义的行为。
//
//因此，我们将所有弱/共享的ptr从列表中复制出来，然后
//开始对它们调用stop（）。保证overlayImpl:：remove（）。
//直到vector<>children离开作用域后才会调用。
