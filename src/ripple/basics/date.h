...
//麻省理工学院执照（MIT）
//
//版权所有（c）2015、2016、2017 Howard Hinnant
//版权所有（c）2016 Adrian Colomitchi
//版权所有（c）2017 Florian Dang
//版权所有（c）2017 Paul Thompson
//
//特此免费向任何获得副本的人授予许可。
...
# if __GNUC__ < 5
//GCC 4.9 bug 61489错误警告-wMissing字段初始值设定项
...
//Oracle Developer Studio 12.6及更早版本
#  define CONSTDATA constexpr const
#  define CONSTCD11 constexpr
#  define CONSTCD14
#  define NOEXCEPT noexcept
...
//施工数据日期：工作日周三
//施工数据日期：工作日周四
//constdata日期：：工作日星期五5U
//施工数据日期：工作日星期六

}  //内联命名空间文本
#endif //！定义（_msc_ver）（_msc_ver>=1900）
...
/*dif/*！已定义（_msc_ver）u msc_ver>=1900*/

    constcd11显式存储时间（std:：chrono:：hours since_dimension）无例外
        ：基本（从午夜开始，从午夜开始，从午夜开始，为24小时）
        {}

    constcd11显式存储时间（std:：chrono:：hours h，unsigned md）无异常
        ：基础（h，h<std:：chrono:：hours 0，md）
        {}

    constcd11 std:：chrono:：hours hours（）const noexcept返回h_
    constcd11 unsigned mode（）const noexcept返回模式_

    constcd14显式运算符precision（）const noexcept
    {
        自动p=to24hr（）；
        如果（负）
            P＝-P；
    ...
    constcd14精度到“duration（）const noexcept”
    {
        返回static_cast<precision>（*this）；
    }

    constcd14 time_of_day_storage&make24（）noexcept_base:：make24（）；return*this；
    constcd14 time_of_day_storage&make12（）noexcept_base:：make12（）；return*this；

    constcd11 bool in_常规_range（）const noexcept
    {
        返回base:：in_常规_range（）；
    }
    ...
    template<class chart，class traits>
    朋友
    std:：basicou ostream<chart，traits>&
    操作员<（std:：basic_ostream<chart，traits>>&os，const time_day_storage&t）
    {
        使用命名空间std；
        详细信息：：保存流<chart，traits>（os）；
        如果（T.Nigi）
            OS< <…
        填充（'0’）；
        os.flags（std:：ios:：dec std:：ios:：right）；
...
template<class rep，class period>
课程时间“天”存储<std:：chrono:：duration<rep，period>，detail:：classify:：minute>
    ：私有详细信息：：时间\天\基础
{
    使用base=detail：：天的时间基数；

    标准：：时间：：分钟

公众：
   使用精度=std:：chrono:：minutes；

   constcd11 time_of_day_storage（）无例外
        基础（）
        ，My（0）
        {}

   constcd11显式存储时间（std:：chrono:：minutes since_dimension）无例外
        ：base（std:：chrono:：duration_cast<std:：chrono:：hours>（从午夜起）
               从午夜开始<std:：chrono:：minutes 0，是24小时）
        ，m_u（细节：：abs（从午夜开始）-h_u）
        {}

    constcd11显式存储时间（std:：chrono:：hours h，std:：chrono:：minutes m，
                                           无符号MD）无例外
        ：基（H，假，MD）
        ，My（m）
        {}
    ...
    constcd11 std:：chrono:：hours hours（）const noexcept返回h_
    constcd11 std:：chrono:：minutes（）const noexcept返回m_
    constcd11 unsigned mode（）const noexcept返回模式_
    ...
    constcd11 bool in_常规_range（）const noexcept
    {
        返回base:：in_-conventional_-range（）&&m_uu<std:：chrono:：hours_1_
    }
...
template<class rep，class period>
课程时间“天存储”<std:：chrono:：duration<rep，period>，detail:：classify:：second>
    ：私有详细信息：：时间\天\基础
{
    使用base=detail：：天的时间基数；
    使用dfs=decimal_format_seconds<std:：chrono:：seconds>；
...
公众：
    ...
    constcd11显式存储时间（std:：chrono:：seconds since_dimension）无例外
        ：base（std:：chrono:：duration_cast<std:：chrono:：hours>（从午夜起）
               从午夜开始<std:：chrono:：seconds 0，是24小时）
        ，m_uu（std:：chrono:：duration_cast<std:：chrono:：minutes>（detail:：abs（从_午夜起）-h_uu））。
        ，S_uu（细节：：abs（从午夜开始）-h_uu-m_u）
        {}

    constcd11显式存储时间（std:：chrono:：hours h，std:：chrono:：minutes m，
                                           std:：chrono:：seconds s s，unsigned md）无异常
        ：基（H，假，MD）
        ，My（m）
        ，S.（S）
        {}
    ...
    constcd11 std:：chrono:：hours hours（）const noexcept返回h_
    constcd11 std:：chrono:：minutes（）const noexcept返回m_
    constcd14 std:：chrono:：seconds&seconds（）noexcept返回s .seconds（）；
    constcd11 std:：chrono:：seconds seconds（）const noexcept返回s_.seconds（）；
    constcd11 unsigned mode（）const noexcept返回模式_

    constcd14显式运算符precision（）const noexcept
    {
        auto p=to24hr（）+s_u.to_duration（）+m_uu；
        如果（负）
            P＝-P；
        返回P；
    }
    ...
    constcd11 bool in_常规_range（）const noexcept
    {
        返回base：：in_conventional_range（）&&m_uu<std:：chrono:：hours 1&&
                                                在传统范围内
    }
    ...
    朋友
    std:：basicou ostream<chart，traits>&
    日期：：到流（std:：basic_ostream<chart，traits>>&os，const chart*fmt，
          const fields<duration>和fds，const std:：string*abbrev，
          const std：：chrono：：seconds*offset_sec）；

    template<class chart，class traits，class duration，class alloc>
    朋友
    std:：basic鏁istream<chart，traits>&
    日期：：来自“流”（std:：basic“流”<chart，traits>>&is，const chart*fmt，
          字段<持续时间>&fds，
          std:：basic_string<chart，traits，alloc>*abbrev，std:：chrono:：minutes*offset）；
}；

template<class rep，class period>
类时间“天存储”<std:：chrono:：duration<rep，period>，detail:：classify:：subsecond>
    ：私有详细信息：：时间\天\基础
{
公众：
    使用duration=std:：chrono:：duration<rep，period>；
    使用dfs=decimal_format_seconds<typename std:：common_type<duration，
                                       标准：：时间：：秒>：：类型>；
    使用precision=typename df:：precision；
...
公众：
    ...
    constcd11显式存储时间（从午夜开始的持续时间）无例外
        ：base（日期：：trunc<std:：chrono:：hours>（从午夜起）
               自午夜<持续时间0，为24小时）
        ，m_uu（日期：：trunc<std:：chrono:：minutes>（细节：：abs（从午夜开始）-h_u））。
        ，S_uu（细节：：abs（从午夜开始）-h_uu-m_u）
        {}

    constcd11显式存储时间（std:：chrono:：hours h，std:：chrono:：minutes m，
                                           std:：chrono:：seconds s s，精度sub_s，
                                           无符号MD）无例外
        ：基（H，假，MD）
        ，My（m）
        ，S*（S+Sube）
    ...
    constcd14 std:：chrono:：seconds&seconds（）noexcept返回s .seconds（）；
    constcd11 std:：chrono:：seconds seconds（）const noexcept返回s_.seconds（）；
    constcd11 precision subseconds（）const noexcept返回s .subseconds（）；
    constcd11 unsigned mode（）const noexcept返回模式_
...
{
    使用base=detail:：time_day_storage<duration>；
公众：
如果有的话！已定义（_msc_ver）_msc_ver>=1900
    constcd11 time_of_day（）noexcept=默认值；
否则
    constcd11 time_of_day（）=默认值；
我很高兴！已定义（_msc_ver）_msc_ver>=1900*/
...
size_t n_might_match = nkw;  //此时，任何关键字都可能匹配
size_t n_does_match = 0;     //但他们都没有
//将所有状态初始化为可能匹配，但“”关键字不匹配
...
//浏览下一张图表，但不要消耗它
        ...
        bool consume = false;
//对于可能匹配的每个关键字，请查看indx字符是否为c
//如果找到匹配项，则使用c
//如果找到匹配项，并且这是关键字中的最后一个字符，
//然后关键字匹配。
//如果关键字与此字符不匹配，则更改关键字
//到不匹配
        st = status;
...
//我们已经退出循环，因为我们达到了EOF和/或我们没有更多的“可能匹配”。
//返回第一个匹配结果
...
//是rdSTATE（）！=ios:：goodbit*fmt==chart
...
#else  //_uu cplusplus<201402（定义（uu edg_u version_uuuu）&&u edg_u version_uuuuuuuux=411）
...
#endif  //_uu cplusplus<201402（定义（uu edg_u version_uuuu）&&u edg_u version_uuuuuuuux=411）
