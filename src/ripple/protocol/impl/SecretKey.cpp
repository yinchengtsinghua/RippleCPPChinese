...
{
private:
Blob gen_; //vvalco编译时大小？

public:
    explicit
    Generator (Seed const& seed)
    {
//Fixme：避免将种子复制到uint128键中
//GenerateRootDeterministicPublicKey复制。
        uint128 ui;
        std::memcpy(ui.data(),
            seed.data(), seed.size());
...
//vfalc todo在不使用openssl的情况下重写所有这些内容
//或要求生成术语
SecretKey
generateSecretKey (KeyType type, Seed const& seed)
{
    ...
    {
//Fixme：避免将种子复制到uint128键中
//GenerateRootDeterministicPrivateKey副本。
        uint128 ps;
        std::memcpy(ps.data(),
            seed.data(), seed.size());
        auto const upk =
