...
Ledger::rawReplace(std::shared_ptr<SLE> const& sle)
{
    ...
    auto item = std::make_shared<
        SHAMapItem const>(sle->key(),
            std::move(ss));
//vfalc note updategiveitem应取得所有权
...
Ledger::setup (Config const& config)
{
    ...
    try
    {
        ...
        {
//vfalc注：为什么getfieldindex和not isfieldpresent？
...
{
if (info_.seq == 0) //Genesis Ledger没有以前的分类帐
        return;
 ...
 *从数据库加载分类帐。
 *
 *@param sqlsuffix:附加到SQL查询的附加字符串。
 （通常是WHERE子句）。
 *@param acquire:如果在本地找不到，则获取分类帐。
 *@返回分类帐、分类帐序列和分类帐哈希。
 **/
