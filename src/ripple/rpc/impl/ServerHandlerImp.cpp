...
//vfalc todo重写以使用boost:：beast:：http:：fields
static
bool
authorized (
    Port const& port,
...
//从请求json中提取请求参数作为“params”。
//
//如果字段“params”为空，“params”是空对象。
//
//否则，该字段必须是长度为1的数组（为什么？）
//我们获取第一个条目并验证它是一个对象。
        Json::Value params;
        if (!batch)
        {
