...
//测试使用数组与写入数据库的值进行比较。
//写入数组的键与
//数据库根据函数generatevalue中的公式。

//数组中保留了从0到标志\u max \u键的空间，值为
//从这些位置随机写入/删除/读取。在验证过程中，我们
//比较数组中的所有位置。缩短/延长跑步距离
//时间，你可以更改设置：标记\u max_key，标记\u ops_per_thread，
//（有时也会标记线程）。
//
//请注意，如果设置了标记“测试批次”快照，则测试将
//不同的行为。有关详细信息，请参见标记的注释。
...
//在释放的Windows模式下不支持同步点。
#if !(defined NDEBUG) || !defined(OS_WIN)
#include "util/sync_point.h"
#endif  //！（定义的ndebug）！定义（OsWin）
...
//必须使智能ptr拥有它，因为端口：：mutex不可复制
//并且将其存储在容器中可能需要根据IMPL进行复制。
...
//给定一个键k和值v，这将放置（“0”+k，“0”+v），“（1”+k，“1”+v），……
//（9”+K，“9”+V），原子量单位为dB，即单个批次。
//另请参阅multiget。
...
//给定一个键k，这将删除（“0”+k），“（1”+k），…（“9”+K）
//在数据库中原子化，即在单个批次中。另请参阅multiget。
...
//给定一个键，它会对“0”+P、“1”+P、…“9”+P进行前缀扫描。
//在同一快照中，p是第一个标志\u前缀\u大小-1字节
//钥匙的钥匙这10次扫描中的每一次都返回一系列值；
//每个系列应具有相同的长度，并对每个系列进行验证。
//索引I：所有i'th值的形式为“0”+V、“1”+V、…“9”+V。
//假设多输出用于输入（k，v）
...
#ifndef ROCKSDB_LITE  //Lite不支持getColumnFamilyMetadata
...
//操作前缀扫描
//键的长度为8字节，前缀大小为标记前缀大小。有
//（8-标记前缀大小）除前缀外的字节。所以将会
//为2^（（8-标记前缀\大小）*8）相同的可能键
//前缀
        if (!FLAGS_test_batches_snapshots) {
