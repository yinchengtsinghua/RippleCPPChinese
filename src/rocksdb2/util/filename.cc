...
//给定一个路径，通过替换所有不在中的字符来展平路径名。
//[0-9，a-z，a-z，-，带。并在末尾附加“_log \0”。
//返回存储在dest中的字符数，不包括尾随的'\0'。
...
rest.remove_prefix(ARCHIVAL_DIR.size() + 1); //添加1以删除/同时
      if (log_type) {
...
Status SetIdentityFile(Env* env, const std::string& dbname) {
  ...
  assert(!id.empty());
//为临时标识文件保留文件名dbname/000000.dbtmp
