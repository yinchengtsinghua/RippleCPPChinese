...
//在这个测试中，我们将<key，value>插入到列族“data”中，并且
//<value，key>to column family`index`。然后按顺序迭代它们
//然后用钥匙寻找它们。
...
batch.SetSavePoint();  //二
  ...
  batch.Put(&cf1, "B", "b2");
  batch.Delete(&cf1, "C");
batch.SetSavePoint();  //三
batch.SetSavePoint();  //四
...
ASSERT_OK(batch.RollbackToSavePoint());  //回滚至4
  ASSERT_EQ(
      "PUT(A):a,PUT(A):aa,DEL(A),PUT(A):aaa,PUT(A):xxx,PUT(B):b,PUT(B):bb,DEL("
      "B)"
      ",PUT(C):cc,",
...
ASSERT_OK(batch.RollbackToSavePoint());  //回滚至3
  ASSERT_EQ(
      "PUT(A):a,PUT(A):aa,DEL(A),PUT(A):aaa,PUT(A):xxx,PUT(B):b,PUT(B):bb,DEL("
      "B)"
      ",PUT(C):cc,",
...
ASSERT_OK(batch.RollbackToSavePoint());  //回滚至2
...
TEST_F(WriteBatchWithIndexTest, SingleDeleteDeltaIterTest) {
  ...
  std::string value;
  DBOptions db_options;
  /*tebatchwithindex批处理（bytewiseComparator（），20，true/*覆盖键*/）；
  成批出售（“A”、“A”）；
  批量放置（“A”、“A2”）；
  批量投放（“b”、“b”）；
  batch.singledelete（“A”）；
