...
void ColumnAwareEncodingReader::InitTableReader(const std::string& file_path) {
  ...
  options_.table_factory->NewTableReader(
      TableReaderOptions(ioptions_, soptions_, internal_comparator_,
                         /*kip_filters=*/false），
      std:：move（file_u），file_size，&table_reader，/*启用预取*/false);
...
uint32_t format_version = 2;  //硬编码版本
  Slice compression_dict;
  *slice_final =
...
void columnawareencodingreader:：getFolderClarationsSecondary（）。
    std:：vector<coldeclaration>**键列声明，
    std:：vector<coldeclaration>>*值列声明，
    冷声明**值_校验和_声明）
  *key_col_declarations=new std:：vector<coldeclaration>
      coldeclaration（“固定长度”，colcompressionType:：kCollRelyVarint，4，false，
                     真的）
      coldeclaration（“固定长度”，colcompressiontype:：kcoldeltavarant，8，
                     假，真）
      coldeclaration（“固定长度”，colcompressiontype:：kcolrledletavarint，8，
                     假，真）
      coldeclaration（“固定长度”，colcompressiontype:：kcolrle，1），
      coldeclaration（“固定长度”，colcompressiontype:：kcoldeltavarant，4，
                     假，真）
      coldeclaration（“固定长度”，colcompressiontype:：kcoldeltavarant，8，
                     假，真）
      coldeclaration（“固定长度”，colcompressionType:：kCollRelyVarint，8，false，
                     真的）
      coldeclaration（“variablechunk”，colcompressiontype:：kcolnocompression），
      coldeclaration（“固定长度”，colcompressiontype:：kcolrlevarint，8）；
  *值_col_declarations=new std:：vector<coldeclaration>（）；
  *value_checksum_declaration=新的冷声明（
      “长固定长度”，colcompressiontype:：kcolnocompression，9，
      真/*可以为空*/);
