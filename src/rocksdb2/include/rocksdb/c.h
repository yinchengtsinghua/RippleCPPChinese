...
/*版权所有（c）2011 LevelDB作者。版权所有。
  ...
  C RocksDB的绑定。可能作为一个稳定的ABI有用
  用于将rocksdb保存在共享库中的程序，或用于
  一个JNI API。
...
//如果值_list[i]=空，错误[i]=空，
//然后我们得到status.isNotFound（），我们不会返回它。
//返回除status.ok（）和status.isNotFound（）之外的所有错误。
//
//errs、values_list和values_list_sizes的长度必须为num_key，
//由呼叫方分配。
//errs是一个字符串列表，与传统的一个错误不同，
//其中，errs[i]是检索键列表[i]的状态。
//每个非空的errs条目都是一个malloc（）ed、以空结尾的字符串。
//每个非空值的列表项都是一个malloc（）ed数组，其中
//存储在值“列出”中的每个值的长度。
...
rocksdb_block_based_table_options_t*, int);  //使用上述枚举之一
extern ROCKSDB_LIBRARY_API void
