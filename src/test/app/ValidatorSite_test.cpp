...
"http://波纹：
"http://ripple.com/验证程序“，
"http://ripple.com:8080/验证程序“，
"http://207.261.33.37/验证程序“，
"http://207.261.33.37:8080/验证程序“，
"https://ripple.com/验证程序“，
"https://ripple.com:443/验证程序“，
...
#if _MSC_VER    //Windows路径从前导/中剥离，使路径为空
//文件URL路径不能是目录
//（/是我们可以合理假设的唯一路径是目录）
badSites[0] = "file://；
        BEAST_EXPECT(!trustedSites->load (badSites));
...
//使用格式错误的JSON创建文件
            ...
            testFileList({
                {goodPath, "" },
{helloPath, "Unable to parse JSON response from  file://“Helopopase}，
{jsonPath, "Missing fields in JSON response from  file://“+ JSONPATION }，
