...
curve25519_add_reduce_packed32(packedelem32 *out, const packedelem32 *r, const packedelem32 *s) {
 ...
 /*=_mm_unpacklo_epi64（r0，r2）；/*00 44*/
 s1=_mm_unpackhi_epi64（r0，r2）；/*11 55*/

 /*=_mm_unpacklo_epi64（r1，r3）；/*22 66*/
 S3=_mm_unpackhi_epi64（r1，r3）；/*33 77*/

 /*=u mm_unpacklo_epi64（_mm_setzero_si128（），r4）；/*00 88*/
 ...
 /*[0].v=_mm_unpacklo_epi64（S0，S1）；/*00 11*/
 out[1].v=_mm_unpacklo_epi64（s2，s3）；/*22 33*/

 /*[2].v=_mm_unpackhi_epi64（S0，S1）；/*44 55*/
 out[3].v=_mm_unpackhi_epi64（s2，s3）；/*66 77*/

 /*[4].v=_mm_unpackhi_epi64（s4，s5）；/*88 99*/
}

Donna_内联静态空隙
曲线25519_add_packed32（packedelem32*out，const packedelem32*r，const packedelem32*s）
...
Donna_内联静态空隙
曲线25519_sub_packed32（packedelem32*out，const packedelem32*r，const packedelem32*s）
 xmmi r0、r1、r2、r3、r4；
 xmmi s0、s1、s2、s3；
 XMMI C1，C2；
...
curve25519_sub_after_basic_packed32(packedelem32 *out, const packedelem32 *r, const packedelem32 *s) {
 ...
 /*=u mm_sub_epi32（r0，s[0].v）；/*00 11*/
 r1=_mm_sub_epi32（r1，s[1].v）；/*22 33*/

 /*=_mm_sub_epi32（r2，s[2].v）；/*44 55*/
 r3=_mm_sub_epi32（r3，s[3].v）；/*66 77*/

 /*=_mm_sub_epi32（r4，s[4].v）；/*88 99*/

 S0=_mm_unpacklo_epi64（r0，r2）；/*00 44*/

 /*=_mm_unpackhi_epi64（r0，r2）；/*11 55*/
 s2=_mm_unpacklo_epi64（r1，r3）；/*22 66*/

 /*=_mm_unpackhi_epi64（r1，r3）；/*33 77*/
 S4=_mm_unpacklo_epi64（_mm_setzero_si128（），r4）；/*00 88*/

 /*=u mm_unpackhi_epi64（_mm_setzero_si128（），r4）；/*00 99*/

 c1=_-mm_-srli_-epi32（s0，26）；c2=_-mm_-srli_-epi32（s2，26）；s0=_-mm_和_-si128（s0，packedmask262626.v）；s2=_-mm_和_-si128（s2，packedmask26262626.v）；s1=_-mm_-add_-epi32（s1，c1）；s3=_-mm_-add_-epi32（s3，c2）；
 
 c1=_-mm_-srli_-epi32（s2，26）；c2=_-mm_-srli_-epi32（s4，26）；s2=_-mm_和_-si128（s2，packedmask262626.v）；s4=_-mm_和_-si128（s4，packedmask26262626 26.v）；s3=_-mm_-add_-epi32（s3，c1）；s5=_-mm_-add_-epi32（s5，c2）；
 c1 = _mm_srli_epi32(s3, 25); c2 = _mm_srli_epi32(s5, 25); s3 = _mm_and_si128(s3, packedmask25252525.v); s5 = _mm_and_si128(s5, packedmask25252525.v); s4 = _mm_add_epi32(s4, c1); s0 = _mm_add_epi32(s0, _mm_or_si128(_mm_slli_si128(c1, 8), _mm_srli_si128(_mm_add_epi32(_mm_add_epi32(_mm_slli_epi32(c2, 4), _mm_slli_epi32(c2, 1)), c2), 8)));
 c1=_-mm_-srli_-epi32（s0，26）；c2=_-mm_-srli_-epi32（s2，26）；s0=_-mm_和_-si128（s0，packedmask262626.v）；s2=_-mm_和_-si128（s2，packedmask26262626.v）；s1=_-mm_-add_-epi32（s1，c1）；s3=_-mm_-add_-epi32（s3，c2）；

 out[0].v=_mm_unpacklo_epi64（S0，S1）；/*00 11*/

 /*[1].v=_mm_unpacklo_epi64（s2，s3）；/*22 33*/
 out[2].v=_mm_unpackhi_epi64（S0，S1）；/*44 55*/

 /*[3].v=_mm_unpackhi_epi64（s2，s3）；/*66 77*/
...
curve25519_contract(unsigned char out[32], const bignum25519 in) {
 ...
 /*现在t在0到2^255-1之间，正确携带。*/
 /*案例1：介于0和2^255-20之间。案例2：在2^255-19和2^255-1之间。*/
	f[0] += 19;
	carry_pass_full()

 /*现在在这两种情况下都在19和2^255-1之间，并被19抵消。*/
	f[0] += (1 << 26) - 19;
	f[1] += (1 << 25) - 1;
	f[2] += (1 << 26) - 1;
	f[3] += (1 << 25) - 1;
 ...
 /*现在介于2^255和2^256-20之间，偏移量为2^255。*/
	carry_pass_final()
