...
ge25519_nielsadd2_p1p1(ge25519_p1p1 *r, const ge25519 *p, const ge25519_niels *q, unsigned char signbit) {
	...
	curve25519_sub(a, p->y, p->x);
	curve25519_add(b, p->y, p->x);
 /*ve25519_mul（a，a，qb[符号位]）；/*x表示+，y表示-*/
 曲线25519_mul（r->x，b，qb[符号^1]）；/*y表示+x表示-*/
 ...
 /*ve25519_-add（rb[2+signbit]，rb[2+signbit]，c）；/*z代表+，t代表-*/
 curve25519_sub（rb[2+（signbit^1）]，rb[2+（signbit^1）]，c）；/*t表示+z表示-*/
...
ge25519_pnielsadd_p1p1(ge25519_p1p1 *r, const ge25519 *p, const ge25519_pniels *q, unsigned char signbit) {
	...
	curve25519_sub(a, p->y, p->x);
	curve25519_add(b, p->y, p->x);
 /*ve25519_mul（a，a，qb[符号]）；/*ysubx for+，xaddy for-*/
 curve25519_mul（r->x，b，qb[符号^1]）；/*xaddy for+，ysubx for-*/
...
ge25519_unpack_negative_vartime(ge25519 *r, const unsigned char p[32]) {
 ...
 /*ve25519_平方（num，r->y）；/*x=y^2*/
 曲线25519_mul（den，num，ge25519_ecd）；/*den=dy^2*/

 /*ve25519_sub_reduce（num，num，r->z）；/*x=y^1-1*/
 ...
 /*2。r->x=num*den^3*的计算（num*den^7）^（（P-5）/8）*/
